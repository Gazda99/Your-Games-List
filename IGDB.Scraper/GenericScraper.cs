...
public class GenericScraper {
    ...
    public async Task<List<T>> Scrape<T>(string rawContent, int lop, int limit) {
        ...
        while (condition) {
            var tasks = new List<Task<ScrapeResult<T>>>();
        ...
        var timeSpan = TimeSpan.FromMilliseconds(totalSw.ElapsedMilliseconds);
        totalSw.Stop();
    ...
    private async Task<ScrapeResult<T>> ScrapeBatch<T>(string rawContent, int offset, int limit, int taskId) {
        var scrapeResult = new ScrapeResult<T>();

        var type = typeof(T).FullName;

        var req = SimpleHttpRequestMsg
